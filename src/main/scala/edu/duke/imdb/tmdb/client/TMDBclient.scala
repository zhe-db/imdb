...
import edu.duke.imdb.tmdb.api.Protocol.{
  AuthenticateResult,
  Configuration,
  Credits,
  Error,
...
import edu.duke.imdb.tmdb.api.TmdbApi
...
object TmdbClient {
  def apply(
      ApiKey: String,
      Language: String = "en",
      tmdbTimeOut: FiniteDuration = 10 seconds
  ): TmdbClient =
...
class TmdbClient(apiKey: String, language: String, tmdbTimeOut: FiniteDuration)
  ...
  def getGenres(): Future[Genres] = {
    tmdbRequest(
      RequestBuilding
        .Get(s"/3/genre/movie/list?${ApiKey}&${Language}")
    )
      .flatMap { response =>
  ...
  def getCredits(id: Long): Future[Credits] = {
    tmdbRequest(
      RequestBuilding.Get(s"/3/movie/${id}/credits?${ApiKey}&${Language}")
    )
      .flatMap { response =>
  ...
  def getCrewDetail(id: Long): Future[Protocol.CrewDetail] = {
    tmdbRequest(
      RequestBuilding.Get(s"/3/person/${id}?${ApiKey}&${Language}")
    ).flatMap { response =>
      Unmarshal(response.entity).to[Protocol.CrewDetail]
    }
  }
  ...
  def getPopularMoviesByPage(page: Int): Future[Results] = {
    tmdbRequest(
      RequestBuilding.Get(
        s"/3/discover/movie?${ApiKey}&${Language}&sort_by=vote_average.desc&include_adult=false&include_video=false&page=${page}"
      )
    ).flatMap { response =>
      Unmarshal(response.entity).to[Results]
    }
  ...
  def getMoviesByYearAndGenre(page: Int, year: Int, genreId: Int): Future[Results] = {
    tmdbRequest(
      RequestBuilding.Get(
        s"/3/discover/movie?${ApiKey}&${Language}&sort_by=vote_average.desc&include_adult=false&include_video=false&page=${page}&year=${year}&with_genres=${genreId}"
      )
    ).flatMap { response =>
      Unmarshal(response.entity).to[Results]
    }
