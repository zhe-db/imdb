...
object RatedMovies {
  ...
  final case class GetRatedMovies(replyTo: ActorRef[RatedMovies])
      extends Command
  final case class AddRatedMovies(item: LineItem) extends Command
  final case class RemoveRatedMovies(productId: String) extends Command

  final case class RatedMovies(movies: Set[LineItem])
  final case class LineItem(productId: String, title: String, quantity: Int)

  private sealed trait InternalCommand extends Command
  private case class InternalGetResponse(
      replyTo: ActorRef[RatedMovies],
        ...
        replicator =>
          ...
          def receiveGetCart: PartialFunction[Command, Behavior[Command]] = {
            ...
            case InternalGetResponse(replyTo, g @ GetSuccess(DataKey, _)) =>
              ...
              val cart = RatedMovies(data.entries.values.toSet)
              replyTo ! cart
              Behaviors.same

            case InternalGetResponse(replyTo, NotFound(DataKey, _)) =>
              replyTo ! RatedMovies(Set.empty)
              Behaviors.same
          ...
          def updateCart(
              data: LWWMap[String, LineItem],
              item: LineItem
          ): LWWMap[String, LineItem] = {
          ...
          def receiveRemoveItem: PartialFunction[Command, Behavior[Command]] = {
            case RemoveRatedMovies(productId) =>
