...
/** Class that contains JDBC source, read parallelism params and target table
  * name
  *
  * @param source
  *   \- JDBC source table
  * @param destination
  *   \- Delta target database.table
  * @param splitBy
  *   \- column by which to split source data while reading
  * @param chunks
  *   \- to how many chunks split jdbc source data
  */
...
/** Class that does reading from JDBC source, transform and writing to Delta
  * table
  *
  * @param jdbcUrl
  *   \- url connecting string for jdbc source
  * @param importConfig
  *   \- case class that contains source read parallelism params and target
  *   table
  * @param jdbcParams
  *   \- additional JDBC session params like isolation level, perf tuning, net
  *   wait params etc...
  * @param dataTransform
  *   \- contains function that we should apply to transform our source data
  */
class JDBCImport(
    jdbcUrl: String,
  ...
  /** obtains lower and upper bound of source table and uses those values to
    * read in a JDBC dataframe
    * @return
    *   a dataframe read from source table
    */
  ...
  /** Runs transform against dataframe read from jdbc and writes it to Delta
    * table
    */
  def run(): Unit = {
    sourceDataframe
